...
import { AuditLogValidationError, AuditLogServiceError } from './service';
...
export class AuditLogController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof AuditLog | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof AuditLogValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof AuditLogServiceError) {
