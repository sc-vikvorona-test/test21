import type { BillingAccountService } from './service';
import { BillingAccountValidationError, BillingAccountServiceError } from './service';
...
export class BillingAccountController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof BillingAccount | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof BillingAccountNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof BillingAccountValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof BillingAccountServiceError) {
