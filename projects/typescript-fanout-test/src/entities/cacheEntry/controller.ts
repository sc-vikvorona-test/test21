import type { CacheEntryService } from './service';
import { CacheEntryValidationError, CacheEntryServiceError } from './service';
...
export class CacheEntryController {
  constructor(private readonly service: CacheEntryService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof CacheEntry | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof CacheEntryNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof CacheEntryValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof CacheEntryServiceError) {
