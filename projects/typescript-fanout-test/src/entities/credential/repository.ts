import type { Credential, CredentialCreate, CredentialUpdate } from './model';
import { makeCredential, updateCredential } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class CredentialRepository {
  private readonly byId = new Map<string, Credential>();
  private nextId = 1;

  all(): Credential[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Credential | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Credential {
    const found = this.byId.get(id);
    if (!found) throw new CredentialNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Credential) => boolean): Credential | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Credential) => boolean): Credential[] {
    const out: Credential[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Credential[] {
    const out: Credential[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: CredentialCreate): Credential {
    const id = `${this.nextId++}`;
    const entity = makeCredential({ ...create, id } as Partial<Credential> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: CredentialUpdate): Credential {
    const current = this.requireById(id);
    const next = updateCredential(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: CredentialCreate, predicate: (existing: Credential) => boolean): Credential {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Credential>(key: K, order: 'asc' | 'desc' = 'asc'): Credential[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: CredentialCreate[]): Credential[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: CredentialUpdate }>): Credential[] {
    const out: Credential[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Credential[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class CredentialNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Credential not found: ${id}`);
    this.name = 'CredentialNotFoundError';
  }
}
