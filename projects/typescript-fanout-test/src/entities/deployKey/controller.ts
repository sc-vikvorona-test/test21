import type { DeployKeyService } from './service';
import { DeployKeyValidationError, DeployKeyServiceError } from './service';
...
export class DeployKeyController {
  constructor(private readonly service: DeployKeyService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof DeployKey | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof DeployKeyNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof DeployKeyValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof DeployKeyServiceError) {
