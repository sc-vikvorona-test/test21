import type { EventRecordService } from './service';
import { EventRecordValidationError, EventRecordServiceError } from './service';
...
export class EventRecordController {
  constructor(private readonly service: EventRecordService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof EventRecord | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof EventRecordNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof EventRecordValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof EventRecordServiceError) {
