...
import { FeatureFlagValidationError, FeatureFlagServiceError } from './service';
...
export class FeatureFlagController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof FeatureFlag | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof FeatureFlagNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof FeatureFlagValidationError) {
