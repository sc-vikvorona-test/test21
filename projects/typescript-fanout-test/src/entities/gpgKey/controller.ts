import type { GpgKeyService } from './service';
import { GpgKeyValidationError, GpgKeyServiceError } from './service';
...
export class GpgKeyController {
  constructor(private readonly service: GpgKeyService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof GpgKey | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof GpgKeyNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof GpgKeyValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof GpgKeyServiceError) {
