import type { HookDeliveryService } from './service';
import { HookDeliveryValidationError, HookDeliveryServiceError } from './service';
...
export class HookDeliveryController {
  constructor(private readonly service: HookDeliveryService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof HookDelivery | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof HookDeliveryValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof HookDeliveryServiceError) {
