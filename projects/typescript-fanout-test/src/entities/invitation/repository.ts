import type { Invitation, InvitationCreate, InvitationUpdate } from './model';
import { makeInvitation, updateInvitation } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class InvitationRepository {
  private readonly byId = new Map<string, Invitation>();
  private nextId = 1;

  all(): Invitation[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Invitation | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Invitation {
    const found = this.byId.get(id);
    if (!found) throw new InvitationNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Invitation) => boolean): Invitation | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Invitation) => boolean): Invitation[] {
    const out: Invitation[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Invitation[] {
    const out: Invitation[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: InvitationCreate): Invitation {
    const id = `${this.nextId++}`;
    const entity = makeInvitation({ ...create, id } as Partial<Invitation> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: InvitationUpdate): Invitation {
    const current = this.requireById(id);
    const next = updateInvitation(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: InvitationCreate, predicate: (existing: Invitation) => boolean): Invitation {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Invitation>(key: K, order: 'asc' | 'desc' = 'asc'): Invitation[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: InvitationCreate[]): Invitation[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: InvitationUpdate }>): Invitation[] {
    const out: Invitation[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Invitation[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class InvitationNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Invitation not found: ${id}`);
    this.name = 'InvitationNotFoundError';
  }
}
