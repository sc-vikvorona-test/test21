...
import { MentionValidationError, MentionServiceError } from './service';
...
export class MentionController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof Mention | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof MentionNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof MentionValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof MentionServiceError) {
