import type { Mention, MentionCreate, MentionUpdate } from './model';
import { makeMention, updateMention } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class MentionRepository {
  private readonly byId = new Map<string, Mention>();
  private nextId = 1;

  all(): Mention[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Mention | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Mention {
    const found = this.byId.get(id);
    if (!found) throw new MentionNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Mention) => boolean): Mention | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Mention) => boolean): Mention[] {
    const out: Mention[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Mention[] {
    const out: Mention[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: MentionCreate): Mention {
    const id = `${this.nextId++}`;
    const entity = makeMention({ ...create, id } as Partial<Mention> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: MentionUpdate): Mention {
    const current = this.requireById(id);
    const next = updateMention(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: MentionCreate, predicate: (existing: Mention) => boolean): Mention {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Mention>(key: K, order: 'asc' | 'desc' = 'asc'): Mention[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: MentionCreate[]): Mention[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: MentionUpdate }>): Mention[] {
    const out: Mention[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Mention[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class MentionNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Mention not found: ${id}`);
    this.name = 'MentionNotFoundError';
  }
}
