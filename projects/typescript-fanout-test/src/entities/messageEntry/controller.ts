import type { MessageEntryService } from './service';
import { MessageEntryValidationError, MessageEntryServiceError } from './service';
...
export class MessageEntryController {
  constructor(private readonly service: MessageEntryService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof MessageEntry | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof MessageEntryNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof MessageEntryValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof MessageEntryServiceError) {
