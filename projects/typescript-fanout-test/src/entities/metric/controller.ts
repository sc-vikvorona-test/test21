import type { MetricService } from './service';
import { MetricValidationError, MetricServiceError } from './service';
...
export class MetricController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof Metric | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof MetricValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof MetricServiceError) {
