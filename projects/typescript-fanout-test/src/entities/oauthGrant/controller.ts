import type { OauthGrantService } from './service';
import { OauthGrantValidationError, OauthGrantServiceError } from './service';
...
export class OauthGrantController {
  constructor(private readonly service: OauthGrantService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof OauthGrant | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof OauthGrantNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof OauthGrantValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof OauthGrantServiceError) {
