import type { Organization, OrganizationCreate, OrganizationUpdate } from './model';
import { makeOrganization, updateOrganization } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class OrganizationRepository {
  private readonly byId = new Map<string, Organization>();
  private nextId = 1;

  all(): Organization[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Organization | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Organization {
    const found = this.byId.get(id);
    if (!found) throw new OrganizationNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Organization) => boolean): Organization | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Organization) => boolean): Organization[] {
    const out: Organization[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Organization[] {
    const out: Organization[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: OrganizationCreate): Organization {
    const id = `${this.nextId++}`;
    const entity = makeOrganization({ ...create, id } as Partial<Organization> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: OrganizationUpdate): Organization {
    const current = this.requireById(id);
    const next = updateOrganization(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: OrganizationCreate, predicate: (existing: Organization) => boolean): Organization {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Organization>(key: K, order: 'asc' | 'desc' = 'asc'): Organization[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: OrganizationCreate[]): Organization[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: OrganizationUpdate }>): Organization[] {
    const out: Organization[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Organization[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class OrganizationNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Organization not found: ${id}`);
    this.name = 'OrganizationNotFoundError';
  }
}
