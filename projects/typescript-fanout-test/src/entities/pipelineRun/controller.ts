import type { PipelineRunService } from './service';
import { PipelineRunValidationError, PipelineRunServiceError } from './service';
...
export class PipelineRunController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof PipelineRun | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof PipelineRunValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof PipelineRunServiceError) {
