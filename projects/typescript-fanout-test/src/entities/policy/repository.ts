import type { Policy, PolicyCreate, PolicyUpdate } from './model';
import { makePolicy, updatePolicy } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class PolicyRepository {
  private readonly byId = new Map<string, Policy>();
  private nextId = 1;

  all(): Policy[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Policy | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Policy {
    const found = this.byId.get(id);
    if (!found) throw new PolicyNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Policy) => boolean): Policy | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Policy) => boolean): Policy[] {
    const out: Policy[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Policy[] {
    const out: Policy[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: PolicyCreate): Policy {
    const id = `${this.nextId++}`;
    const entity = makePolicy({ ...create, id } as Partial<Policy> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: PolicyUpdate): Policy {
    const current = this.requireById(id);
    const next = updatePolicy(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: PolicyCreate, predicate: (existing: Policy) => boolean): Policy {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Policy>(key: K, order: 'asc' | 'desc' = 'asc'): Policy[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: PolicyCreate[]): Policy[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: PolicyUpdate }>): Policy[] {
    const out: Policy[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Policy[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class PolicyNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Policy not found: ${id}`);
    this.name = 'PolicyNotFoundError';
  }
}
