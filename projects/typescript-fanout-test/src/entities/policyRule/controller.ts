import type { PolicyRuleService } from './service';
import { PolicyRuleValidationError, PolicyRuleServiceError } from './service';
...
export class PolicyRuleController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof PolicyRule | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof PolicyRuleNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof PolicyRuleValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof PolicyRuleServiceError) {
