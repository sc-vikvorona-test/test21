...
import { PullRequestValidationError, PullRequestServiceError } from './service';
...
export class PullRequestController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof PullRequest | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof PullRequestValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof PullRequestServiceError) {
