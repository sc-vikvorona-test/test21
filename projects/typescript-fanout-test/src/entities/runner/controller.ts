...
import { RunnerValidationError, RunnerServiceError } from './service';
...
export class RunnerController {
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof Runner | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  ...
  if (err instanceof RunnerValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof RunnerServiceError) {
