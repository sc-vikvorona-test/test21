import type { Schedule, ScheduleCreate, ScheduleUpdate } from './model';
import { makeSchedule, updateSchedule } from './model';

/**
 * In-memory store. Methods are synchronous on purpose — this is a fixture, not a database.
 * Realistic data shapes; not derived from any external project.
 */
export class ScheduleRepository {
  private readonly byId = new Map<string, Schedule>();
  private nextId = 1;

  all(): Schedule[] {
    return Array.from(this.byId.values());
  }

  count(): number {
    return this.byId.size;
  }

  findById(id: string): Schedule | undefined {
    return this.byId.get(id);
  }

  requireById(id: string): Schedule {
    const found = this.byId.get(id);
    if (!found) throw new ScheduleNotFoundError(id);
    return found;
  }

  findFirst(predicate: (entity: Schedule) => boolean): Schedule | undefined {
    for (const entity of this.byId.values()) {
      if (predicate(entity)) return entity;
    }
    return undefined;
  }

  filter(predicate: (entity: Schedule) => boolean): Schedule[] {
    const out: Schedule[] = [];
    for (const entity of this.byId.values()) {
      if (predicate(entity)) out.push(entity);
    }
    return out;
  }

  paginate(offset: number, limit: number): Schedule[] {
    const out: Schedule[] = [];
    let i = 0;
    for (const entity of this.byId.values()) {
      if (i >= offset && i < offset + limit) out.push(entity);
      i++;
      if (i >= offset + limit) break;
    }
    return out;
  }

  insert(create: ScheduleCreate): Schedule {
    const id = `${this.nextId++}`;
    const entity = makeSchedule({ ...create, id } as Partial<Schedule> & { id: string });
    this.byId.set(id, entity);
    return entity;
  }

  update(id: string, patch: ScheduleUpdate): Schedule {
    const current = this.requireById(id);
    const next = updateSchedule(current, patch);
    this.byId.set(id, next);
    return next;
  }

  upsert(create: ScheduleCreate, predicate: (existing: Schedule) => boolean): Schedule {
    const existing = this.findFirst(predicate);
    if (existing) return existing;
    return this.insert(create);
  }

  delete(id: string): boolean {
    return this.byId.delete(id);
  }

  clear(): void {
    this.byId.clear();
    this.nextId = 1;
  }

  sortedBy<K extends keyof Schedule>(key: K, order: 'asc' | 'desc' = 'asc'): Schedule[] {
    const items = this.all();
    const dir = order === 'asc' ? 1 : -1;
    items.sort((a, b) => {
      const av = a[key] as unknown;
      const bv = b[key] as unknown;
      if (av === bv) return 0;
      if (av === null || av === undefined) return -dir;
      if (bv === null || bv === undefined) return dir;
      return (av as number) < (bv as number) ? -dir : dir;
    });
    return items;
  }

  batchInsert(items: ScheduleCreate[]): Schedule[] {
    return items.map((item) => this.insert(item));
  }

  batchUpdate(updates: Array<{ id: string; patch: ScheduleUpdate }>): Schedule[] {
    const out: Schedule[] = [];
    for (const u of updates) {
      try { out.push(this.update(u.id, u.patch)); } catch { /* swallow missing */ }
    }
    return out;
  }

  replaceAll(items: Schedule[]): void {
    this.byId.clear();
    for (const item of items) this.byId.set(item.id, item);
  }
}

export class ScheduleNotFoundError extends Error {
  constructor(public readonly id: string) {
    super(`Schedule not found: ${id}`);
    this.name = 'ScheduleNotFoundError';
  }
}
