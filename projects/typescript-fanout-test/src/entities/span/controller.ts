import type { SpanService } from './service';
import { SpanValidationError, SpanServiceError } from './service';
...
export class SpanController {
  constructor(private readonly service: SpanService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof Span | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof SpanNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof SpanValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof SpanServiceError) {
