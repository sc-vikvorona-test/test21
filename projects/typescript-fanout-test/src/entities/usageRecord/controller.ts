import type { UsageRecordService } from './service';
import { UsageRecordValidationError, UsageRecordServiceError } from './service';
...
export class UsageRecordController {
  constructor(private readonly service: UsageRecordService) {}
  ...
  handleList(req: HttpRequest): HttpResponse {
    ...
    const sortBy = req.query.sortBy as keyof UsageRecord | undefined;
...
function errorToResponse(err: unknown): HttpResponse {
  if (err instanceof UsageRecordNotFoundError) {
    return { status: 404, body: { error: err.message } };
  }
  if (err instanceof UsageRecordValidationError) {
    return { status: 422, body: { error: err.message, fields: err.errors } };
  }
  if (err instanceof UsageRecordServiceError) {
